...
#         return result.document.export_to_markdown()
#     except ImportError:
#         # Docling not installed — attempt plain text read
#         try:
...
#         raise HTTPException(status_code=503, detail="Azure OpenAI client not initialised. Check environment variables.")
...
# def build_styles() -> dict:
#     return {
#         "title": ParagraphStyle("ReportTitle", fontName="Helvetica-Bold", fontSize=26, leading=32, textColor=C_WHITE, alignment=TA_LEFT),
#         "subtitle": ParagraphStyle("Subtitle", fontName="Helvetica", fontSize=11, leading=14, textColor=C_MUTED, alignment=TA_LEFT),
#         "section": ParagraphStyle("Section", fontName="Helvetica-Bold", fontSize=13, leading=16, textColor=C_ACCENT, spaceBefore=6, spaceAfter=4, letterSpacing=1),
#         "body": ParagraphStyle("Body", fontName="Helvetica", fontSize=10, leading=15, textColor=C_TEXT),
#         "kpi_value": ParagraphStyle("KPIValue", fontName="Helvetica-Bold", fontSize=18, leading=22, textColor=C_WHITE, alignment=TA_CENTER),
#         "kpi_label": ParagraphStyle("KPILabel", fontName="Helvetica", fontSize=8, leading=10, textColor=C_MUTED, alignment=TA_CENTER),
#         "insight_title": ParagraphStyle("InsightTitle", fontName="Helvetica-Bold", fontSize=11, leading=14, textColor=C_WHITE),
#         "insight_body": ParagraphStyle("InsightBody", fontName="Helvetica", fontSize=9, leading=13, textColor=C_TEXT, spaceAfter=4),
#         "tag": ParagraphStyle("Tag", fontName="Helvetica-Bold", fontSize=8, leading=10, textColor=C_ACCENT),
#         "rec": ParagraphStyle("Rec", fontName="Helvetica", fontSize=10, leading=14, textColor=C_TEXT, leftIndent=12),
#         "footer": ParagraphStyle("Footer", fontName="Helvetica", fontSize=8, leading=10, textColor=C_MUTED, alignment=TA_CENTER),
#     }
...
#     buffer = io.BytesIO()
#     st = build_styles()
#     doc = SimpleDocTemplate(buffer, pagesize=A4, leftMargin=20*mm, rightMargin=20*mm, topMargin=20*mm, bottomMargin=22*mm)
#     story = []
#     W = A4[0] - 40 * mm
...
#     for i, kpi in enumerate(report.kpis[:8]):
#         trend_char = "▲" if kpi.trend == "up" else "▼" if kpi.trend == "down" else "▶"
#         trend_color = "22c55e" if kpi.trend == "up" else "ef4444" if kpi.trend == "down" else "64748b"
#         cell = [
#             Paragraph(kpi.value, st["kpi_value"]),
#             Spacer(1, 1 * mm),
#             Paragraph(kpi.label.upper(), st["kpi_label"]),
#             Spacer(1, 1 * mm),
#             Paragraph(f'<font color="#{trend_color}">{trend_char} {kpi.score}/100</font>', st["kpi_label"]),
#         ]
#         row.append(cell)
#         if len(row) == 4 or i == len(report.kpis) - 1:
#             while len(row) < 4:
...
#     story.append(Spacer(1, 2 * mm))
#     for ins in report.insights:
#         priority_color = "#ef4444" if ins.priority == "HIGH" else "#f59e0b" if ins.priority == "MEDIUM" else "#64748b"
#         header_text = f'<font color="{priority_color}">● {ins.priority}</font>  <font color="#4f8ef7">{ins.category.upper()}</font>'
#         cell_content = [
#             Paragraph(header_text, st["tag"]), Spacer(1, 2*mm),
#             Paragraph(ins.title, st["insight_title"]), Spacer(1, 2*mm),
#             Paragraph(ins.body, st["insight_body"]),
#         ]
...
#     rec_rows = []
#     for i, rec in enumerate(report.recommendations, 1):
#         num_cell = Paragraph(f'<font color="#4f8ef7"><b>{i}</b></font>',
#             ParagraphStyle("Num", fontName="Helvetica-Bold", fontSize=12, leading=14, textColor=C_ACCENT, alignment=TA_CENTER))
...
#     story.append(HRFlowable(width=W, thickness=0.5, color=C_MUTED))
#     story.append(Spacer(1, 2 * mm))
#     story.append(Paragraph("Powered by Docling · Azure OpenAI (GPT) · ReportLab  |  Document Intelligence Platform", st["footer"]))
...
# def analyze_document_from_file(file_bytes: bytes, filename: str, question: str = "") -> InsightReport:
