...
def create_task(body_str, user_id):
    ...
    if priority not in valid_priorities:
        return error_response(
            f"Priority must be one of: {', '.join(valid_priorities)}", 400
        )
    ...
    if issue_type not in valid_issue_types:
        return error_response(
            f"Issue type must be one of: {', '.join(valid_issue_types)}", 400
        )
    ...
    if assignee_id:
        ...
        if assignee:
            ...
            if creator_role == "member" and assignee_role in ["admin", "super-admin"]:
                return error_response(
                    "Members cannot assign tasks to admin or super-admin users", 403
                )
        ...
        manager.broadcast_to_channel(
            {
                "type": "task_created",
                "task": task,
                "user_name": User.find_by_id(user_id).get("name", "Unknown")
    ...
    return success_response({"message": "Task created successfully", "task": task}, 201)
...
def get_project_tasks(project_id, user_id):
    ...
    tasks_list = list(
        db.tasks.find({"project_id": project_id}, task_projection).sort(
            "created_at", -1
        )
    )
    ...
    sprint_ids = list(
        set([task["sprint_id"] for task in tasks_list if task.get("sprint_id")])
    )
    creator_ids = list(
        set([task["created_by"] for task in tasks_list if task.get("created_by")])
    )
...
def update_task(body_str, task_id, user_id):
    ...
    if "title" in data:
        if (
            data["title"] is not None and data["title"].strip()
    ...
    if (
        "priority" in data and data["priority"]
    ):  # Only validate if priority is not None/empty
        valid_priorities = ["Low", "Medium", "High"]
    ...
    if "status" in data:
        ...
        if data["status"] == "Closed":
            ...
            update_data["approved_by_name"] = (
                current_user["name"] if current_user else "Unknown"
            )
    ...
    if success:
        ...
        # Convert moved_to_backlog_at if present
        if (
            "moved_to_backlog_at" in updated_task
            and updated_task["moved_to_backlog_at"]
        ):
...
def add_label_to_task(task_id, body_str, user_id):
    ...
    if success:
        ...
        return success_response(
            {"message": "Label already exists on this task", "label": label}
        )
...
def get_project_labels(project_id, user_id):
    ...
    return success_response({"labels": sorted(list(labels))})
...
def add_attachment_to_task(task_id, body_str, user_id):
    ...
    if not is_base64 and not is_http_url:
        return error_response(
            "URL must start with http://, https://, or be a base64 data URL", 400
        )
    ...
    if success:
        ...
        attachment_type = (
            "link" if url.startswith(("http://", "https://")) else "document"
        )
        activity_data = {
            "user_id": user_id,
            "user_name": user["name"],
...
def add_link_to_task(task_id, body_str, user_id):
    ...
    if not linked_task_id and not linked_ticket_id:
        return error_response(
            "Missing required field: linked_task_id or linked_ticket_id", 400
        )
    ...
    if link_type not in valid_link_types:
        return error_response(
            f"Link type must be one of: {', '.join(valid_link_types)}", 400
        )
    ...
    if linked_ticket_id:
        ...
        if not linked_task:
            return error_response(
                f"Task with ticket ID '{linked_ticket_id}' not found", 404
            )
    ...
    if not Project.is_member(linked_task["project_id"], user_id):
        return error_response(
            "Access denied. You are not a member of the linked task's project.", 403
        )
    ...
    if success:
        ...
        return success_response({"message": "Link added successfully", "link": link})
    else:
    ...
    if success:
        ...
        if link_type in reverse_link_map:
            ...
            if linked_task:
                ...
                Task.remove_link(
                    linked_task["_id"], task["ticket_id"], reverse_link_type
                )

        return success_response({"message": "Link removed successfully"})
    else:
...
def approve_task(task_id, user_id):
    ...
    update_data = {
        ...
        .replace(tzinfo=None)
        .isoformat(),  # Add closed_at as well
        "in_backlog": False,
    ...
    if success:
        ...
        return success_response(
            {"message": "Task approved and closed successfully", "task": updated_task}
        )
    else:
...
def get_done_tasks_for_approval(project_id, user_id):
    ...
    if project["user_id"] != user_id:
        return error_response(
            "Only project owner can view tasks awaiting approval", 403
        )

    # Get all tasks with Done status for this project
    from database import tasks

    done_tasks_cursor = tasks.find({"project_id": project_id, "status": "Done"}).sort(
        "updated_at", -1
    )
    ...
    return success_response({"tasks": done_tasks, "count": len(done_tasks)})
...
def get_all_pending_approval_tasks(user_id):
    ...
    if user_role in ["admin", "super-admin"]:
        ...
        tasks_cursor = db.tasks.find(
            {"project_id": {"$in": project_ids}, "status": "Done"}, task_projection
        ).sort("updated_at", -1)
        ...
        tasks_cursor = db.tasks.find(
            {"assignee_id": user_id, "status": "Done"}, task_projection
        ).sort("updated_at", -1)
    ...
    return success_response(
        {"tasks": pending_tasks, "count": len(pending_tasks), "user_role": user_role}
    )
...
def get_all_closed_tasks(user_id):
    ...
    if user_role in ["admin", "super-admin"]:
        ...
        tasks_cursor = db.tasks.find(
            {"project_id": {"$in": project_ids}, "status": "Closed"}, task_projection
        ).sort("closed_at", -1)
        ...
        tasks_cursor = db.tasks.find(
            {"assignee_id": user_id, "status": "Closed"}, task_projection
        ).sort("closed_at", -1)
    ...
    return success_response(
        {"tasks": closed_tasks, "count": len(closed_tasks), "user_role": user_role}
    )
