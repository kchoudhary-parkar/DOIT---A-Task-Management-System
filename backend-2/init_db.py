...
"""
Database initialization script
Creates the hardcoded super-admin account and Azure AI Agent if they don't exist
Initializes default chat channels for projects
"""
...
def initialize_super_admin():
    ...
    if existing_super_admin:
        ...
        if existing_super_admin.get("role") != "super-admin":
            users.update_one(
                {"email": SUPER_ADMIN_EMAIL}, {"$set": {"role": "super-admin"}}
            )
            print(f"✓ Updated role to super-admin for: {SUPER_ADMIN_EMAIL}")
    else:
        # Create the super-admin account
...
def initialize_azure_agent():
    """
    Create Azure AI Agent service account for automation
    """
    ...
    if existing_agent:
        print(f"✓ Azure AI Agent account already exists: {AGENT_EMAIL}")
    else:
        now = datetime.now(timezone.utc).replace(tzinfo=None)
        ...
        print(f"✓ Created Azure AI Agent account: {AGENT_EMAIL}")
        print(f"  Agent ID: {result.inserted_id}")
        print(f"  ⚠️  Run 'python init_agent.py' to configure authentication token")
...
def initialize_default_channels():
    ...
    try:
        ...
        for project in projects:
            ...
            # Check if project already has channels
            existing_channels = db.chat_channels.count_documents(
                {"project_id": project_id}
            )
            ...
            if existing_channels == 0:
                ...
                channels_created += 1
                print(
                    f"✓ Created 'general' channel for project: {project.get('name', 'Unknown')}"
                )
