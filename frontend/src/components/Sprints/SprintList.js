...
// frontend/src/components/Sprints/SprintList.js
...
const SprintList = ({ sprints, projectId, isOwner, onStart, onComplete, onDelete, onRefresh, onAddTask, backlogTasks = [], availableTasks = [], sprintTasks = {} }) => {
  ...
  const getProgressPercentage = (completedTasks, totalTasks) => {
    // Handle edge case: no tasks in sprint
  ...
  return (
    <div className="sprint-list">
      {sprints.map((sprint) => {
        ...
        // FIX: Properly handle progress calculation to avoid NaN
        const totalTasksForProgress = taskCount > 0 ? taskCount : (sprint.total_tasks || 0);
        const completedTasksForProgress = sprint.completed_tasks || 0;
        
        // Use updated getProgressPercentage that handles zero case
        const progress = getProgressPercentage(completedTasksForProgress, totalTasksForProgress);
          ...
          <div key={sprint._id} className={`sprint-card sprint-${sprint.status}`}>
            ...
            <div className="sprint-progress">
              <div className="progress-info">
                <span className="progress-text">
                  <FiTrendingUp size={14} style={{ marginRight: '6px', verticalAlign: 'middle' }} />
                  {completedTasksForProgress} / {totalTasksForProgress} tasks completed
                </span>
                <span className="progress-percentage">{progress}%</span>
              </div>
              <div className="progress-bar">
