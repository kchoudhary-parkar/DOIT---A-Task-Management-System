      ...
      <button
        ...
        onMouseEnter={(e) => {
          if (!isLoading) {
            ...
            e.currentTarget.style.boxShadow = '0 8px 24px rgba(79,142,247,0.4)';
          }
        }}
        onMouseLeave={(e) => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    borderBottom: index < exportOptions.length - 1 ? '1px solid var(--dash-border-primary)' : 'none',
                    cursor: isLoading ? 'not-allowed' : 'pointer',
                    transition: 'all 0.2s',
                    fontSize: '14px',
...
function DashboardPage() {
  ...
  const visibleUpcomingDeadlines = useMemo(
    () => (analytics?.upcoming_deadlines || []).slice(0, 6),
    [analytics?.upcoming_deadlines]
  );

  const visibleRecentActivities = useMemo(
    () => (analytics?.recent_activities || []).slice(0, 6),
  ...
  <div className="project-stat-col">
    <div
      ...
      style={{ ...projectCardBaseStyle, cursor: 'pointer' }}
      onMouseEnter={e => {
  ...
  <div className="project-stat-col">
    ...
    >
      ...
      <div style={{ fontSize: 14, color: 'var(--text-secondary)', minHeight: 20, display: 'flex', alignItems: 'center' }}>
        Owned Projects
      </div>
    </div>
  </div>
          ...
          <div className="deadlines-section-new">
            ...
            {analytics.upcoming_deadlines.length === 0 ? (
              ...
              <div className="deadlines-list">
                {visibleUpcomingDeadlines.map((task, index) => (
                  <div
                    key={index}
